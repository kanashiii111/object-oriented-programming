...
class PointGuard(Base):
    ...
    id: Mapped[int] = mapped_column(
        ForeignKey("players.id"), primary_key=True, autoincrement=False
    )
    ...
    def __init__(
        ...
        assists=None,
        three_point_makes=None,
    ):
        self.id = id

        if assists is None and three_point_makes is None:
            self.assists = 111
            self.three_point_makes = 111
        elif assists is None and three_point_makes is not None:
            self.assists = 222
            self.three_point_makes = three_point_makes
        elif assists is not None and three_point_makes is None:
            self.assists = assists
            self.three_point_makes = 333
        else:
            self.assists = assists or 0
            self.three_point_makes = three_point_makes or 0
    ...
    def play(self):
        return str.format(
            "{name} is orchestrating the offense, making plays and hitting three-pointers.",
            name=self.getPlayer().getName(),
        )

    def train(self):
        return str.format(
            "{name} is training on ball handling, passing and shooting three-pointers.",
            name=self.getPlayer().getName(),
        )
    ...
    def makeThreePointShot(self):
        ...
        return str.format(
            "Three point makes: {three_point_makes}",
            three_point_makes=self.three_point_makes,
        )

    def makePass(self):
        return str.format(
            "{name} is passing the ball to a teammate.", name=self.getPlayer().getName()
        )

    def dribble(self):
        return str.format(
            "{name} is dribbling the ball.", name=self.getPlayer().getName()
        )
